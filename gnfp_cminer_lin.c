 ...
 *
 * Dual-connection 5% fee (official wallet style)
...
/* fee connection */
static int g_fee_fd = -1;
static SSL *g_fee_ssl = NULL;
...
static int net_connect_main(const cfg_t *cfg) {
    ...
    if (cfg->tls) {
        ...
        if (SSL_connect(ssl) != 1) {
            fprintf(stderr, "main TLS handshake failed\n");
            SSL_free(ssl);
            close(fd);
            return -1;
        }
...
static void session_loop(const cfg_t *cfg) {
    while (g_run) {
        ...
        while (g_run) {
            ...
            if (g_fee_fd >= 0)
                pump_reads_fd(g_fee_fd, g_fee_ssl, cfg);   /* drain fee replies */
            flush_submits(cfg);
...
static void usage(void) {
    printf(
        "GNFPHash C miner %s (declared %d%% dual-connection fee)\n"
        ...
        "  --selftest\n"
        "  --bench SECONDS\n"
        "Fee %d%% time-sliced on second connection to %s\n",
        VERSION, DEV_FEE_PCT, DEFAULT_HOST, DEFAULT_PORT,
        DEV_FEE_PCT, DEV_FEE_ADDR);
}
